...
<section class="content">
    ...
    <div class="row">
            <div class="col-lg-7 col-md-6 col-sm-12">
                <h2>Source
                <small class="text-muted">Welcome to Source List</small>
                </h2>
            </div>
...
$(document).ready(function(){ 
    ...
    function fetch_data(){
        $.ajax({
            url:"<?php echo base_url();?>index.php/masters/source/getlist",
            method:"POST",
            dataType:"json",
            success:function(data){
                ...
                {
                    html += '<tr>';
                    html += '<td><input type="checkbox" id="'+data[count].id+'" s_no="'+data[count].s_no+'" data-source_name="'+data[count].source_name+'" data-source_status="'+data[count].source_status+'" class="check_box"  /></td>';
                    html += '<td>'+data[count].s_no+'</td>';
                    html += '<td>'+data[count].source_name+'</td>';
                    if(data[count].source_status == 1){
    ...
    $(document).on('click', '.check_box', function(){
        ...
        if(this.checked)
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'" s_no="'+$(this).attr('s_no')+'" data-source_name="'+$(this).data('source_name')+'" data-source_status="'+$(this).data('source_status')+'" class="check_box" checked /></td>';
            html += '<td>'+$(this).attr('s_no')+'</td>';
            html += '<td><input type="text" name="source_name[]" class="form-control field_required form-control-bg-white" value="'+$(this).data("source_name")+'" /></td>';
            html += '<td><select name="source_status[]" id="source_status_'+$(this).attr('id')+'" class="form-control field_required form-control-bg-white"><option value="1">Active</option><option value="0">IN-active</option></select><input type="hidden" name="hidden_id[]" value="'+$(this).attr('id')+'" /></td>';
        ...
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'" s_no="'+$(this).attr('s_no')+'" data-source_name="'+$(this).data('source_name')+'" data-source_status="'+$(this).data('source_status')+'" class="check_box" /></td>';
            html += '<td>'+$(this).attr('s_no')+'</td>';
            html += '<td>'+$(this).data('source_name')+'</td>';
            if($(this).data('source_status') == 1){
                html += '<td><lable class="btn btn-success small-label">Active</label></td>';
            }else{
                html += '<td><lable class="btn btn-danger small-label">In-Active</label></td>';
            }
        ...
        $('#source_status_'+$(this).attr('id')+'').val($(this).data('source_status'));
    });
});
...
function deleteRecord(id){
    if (confirm('Are you sure you want to delete this item?')) {
          $.ajax({
          url:"<?php echo base_url();?>index.php/masters/source/daleteRecord",
          method:"POST",
          data:'id='+id,
...
function update(){
    ...
    if(flag == true){
         if($('.check_box:checked').length > 0){
             $.ajax({
               url:"<?php echo base_url();?>index.php/masters/source/update",
               method:"POST",
...
function addItemRow(){
    ...
    html += '<td></td>';
    html += '<td><input type="text" name="source_name[]" class="form-control field_required form-control-bg-white" /></td>';
    html += '<td><select name="source_status[]" class="form-control  form-control-bg-white"><option value="1">Active</option><option value="0">IN-active</option></select><input type="hidden" name="hidden_id[]" value="0" /></td>';
