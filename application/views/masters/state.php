...
$(document).ready(function(){ 
	...
	function fetch_data(){
        $.ajax({
            url:"<?php echo base_url();?>index.php/masters/state/getlist",
            method:"POST",
            dataType:"json",
            success:function(data){
                ...
                {
                    html += '<tr>';
                    html += '<td><input type="checkbox" s_no="'+data[count].s_no+'" id="'+data[count].id+'" data-state_name="'+data[count].state_name+'" data-state_status="'+data[count].state_status+'" class="check_box"  /></td>';
					html += '<td>'+data[count].s_no+'</td>';
                    html += '<td>'+data[count].state_name+'</td>';
					if(data[count].state_status == 1){
	...
	$(document).on('click', '.check_box', function(){
		...
		if(this.checked)
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'" s_no="'+$(this).attr('s_no')+'"  data-state_name="'+$(this).data('state_name')+'" data-state_status="'+$(this).data('state_status')+'" class="check_box" checked /></td>';
			html += '<td>'+$(this).attr('s_no')+'</td>';
            html += '<td><input type="text" name="state_name[]" class="form-control field_required form-control-bg-white" value="'+$(this).data("state_name")+'" /></td>';
            html += '<td><select name="state_status[]" id="state_status_'+$(this).attr('id')+'" class="form-control field_required form-control-bg-white"><option value="1">Active</option><option value="0">IN-active</option></select><input type="hidden" name="hidden_id[]" value="'+$(this).attr('id')+'" /></td>';
        ...
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'" s_no="'+$(this).attr('s_no')+'" data-state_name="'+$(this).data('state_name')+'" data-state_status="'+$(this).data('state_status')+'" class="check_box" /></td>';
			html += '<td>'+$(this).attr('s_no')+'</td>';
            html += '<td>'+$(this).data('state_name')+'</td>';
			if($(this).data('state_status') == 1){
				html += '<td><lable class="btn btn-success small-label">Active</label></td>';
			}else{
				html += '<td><lable class="btn btn-danger small-label">In-Active</label></td>';
			}
        ...
        $('#state_status_'+$(this).attr('id')+'').val($(this).data('state_status'));
    });
});
...
function update(){
	...
	if(flag == true){
		 if($('.check_box:checked').length > 0){
			 $.ajax({
			   url:"<?php echo base_url();?>index.php/masters/state/update",
			   method:"POST",
...
function addItemRow(){
	...
	html += '<td></td>';
    html += '<td><input type="text" name="state_name[]" class="form-control field_required form-control-bg-white" /></td>';
    html += '<td><select name="state_status[]" class="form-control  form-control-bg-white"><option value="1">Active</option><option value="0">IN-active</option></select><input type="hidden" name="hidden_id[]" value="0" /></td>';
