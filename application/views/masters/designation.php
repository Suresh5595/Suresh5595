...
<section class="content">
    ...
    <div class="row">
            <div class="col-lg-7 col-md-6 col-sm-12">
                <h2>Designation
                <small class="text-muted">Welcome to Designation List</small>
                </h2>
            </div>
							...
							<?php if($results){ ?>
								<?php foreach($results as $key => $val){ ?>
									<tr>
										...
										<td><?php echo get_val('dep_name','dep_id',$val->department_id_fk,'rsm_department');?></td>
										<td><?php echo $val->des_name;?></td>
										<td>
											<?php if($val->des_status == 0) { ?>
												<lable class="btn btn-success small-label">Active</label>
											<?php }else{ ?>
												<lable class="btn btn-danger small-label">In-Active</label>
											<?php }?>
										</td>
										<td><a href="javascript:void(0);" onclick="edit(<?php echo $val->des_id;?>)" ><span class="badge badge-info">EDIT</span></a>&numsp;<a href="javascript:void(0);" onclick="deleteRecord(<?php echo $val->des_id;?>)"><span class="badge badge-danger">DELETE</span></a></td>
									</tr>
								<?php } ?>
							<?php } ?>
                            </tbody>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-header" style="border-bottom-style: solid;border-bottom-width: 1px;padding-top: 10px;padding-left: 10px;padding-right: 10px;">
				...
				<h2 style="font-size: 15px;color: #1fa7b2;margin-bottom: 10px;"><strong id="popup_title">ADD Designation</strong></h2>
			</div>

			<div class="modal-body">
			<form method="post" onSubmit="return form_validate();" id="designation_form" action="<?php echo base_url();?>index.php/masters/designation/update">
				<input type="hidden" name="hidden_id" value="0" id="hidden_id">
				<div class="row clearfix">
					<div class="col-sm-12">
						<b>Department</b>
						<div class="form-group">
							<select class="form-control field_required" name="department_id" id="department_id">
							<option value="">-Select-</option>
								<?php if($departments){ ?>
									<?php foreach($departments as $dep_val){ ?>
										<option value="<?php echo $dep_val->dep_id;?>"><?php echo $dep_val->dep_name;?></option>
									<?php } ?>
								<?php } ?>
							</select>
						</div>
				...
				<div class="row clearfix">
					<div class="col-sm-12">
						...
						<div class="form-group">
							<input type="text" class="form-control field_required" id="designation_name" name="designation_name">
						</div>
					</div>
				</div>
				<div class="row clearfix">
...
function edit(id){
    ...
    type: "POST",
    cache : false,
    url: "<?php echo base_url(); ?>index.php/masters/designation/edit",
    data: dataString,
    ashync: false,
    dataType:'json',
    success: function(data){ 
         $('#myModal2').modal({ backdrop: 'static',keyboard: true,show: true });
         $('#hidden_id').val(data.des_id); 
		 $('#department_id').val(data.department_id_fk);
         $('#designation_name').val(data.des_name);
         $('#designation_status').val(data.des_status);
       }
     });
}
...
function addRow(){
	$('#popup_title').html('');
	$('#popup_btn').html('');
	$('#popup_title').html('ADD DESIGNATION');
	$('#popup_btn').html('SAVE');
	$('#hidden_id').val(0); 
	$('#department_id').val('');
    $('#designation_name').val('');
...
function deleteRecord(id){
    ...
    type: "POST",
    cache : false,
    url: "<?php echo base_url(); ?>index.php/masters/designation/daleteRecord",
    data: dataString,
    ashync: false,
    dataType:'json',
    success: function(data){ 
