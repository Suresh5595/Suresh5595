...
$(document).ready(function(){ 
	...
	function fetch_data(){
        $.ajax({
            url:"<?php echo base_url();?>index.php/masters/unit/getlist",
            method:"POST",
            dataType:"json",
            success:function(data){
                ...
                {
                    html += '<tr>';
                    html += '<td><input type="checkbox" s_no="'+data[count].s_no+'" id="'+data[count].id+'" data-unit_name="'+data[count].unit_name+'" data-unit_status="'+data[count].unit_status+'" class="check_box"  /></td>';
					html += '<td>'+data[count].s_no+'</td>';
                    html += '<td>'+data[count].unit_name+'</td>';
					if(data[count].unit_status == 1){
	...
	$(document).on('click', '.check_box', function(){
		...
		if(this.checked)
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'" s_no="'+$(this).attr('s_no')+'"  data-unit_name="'+$(this).data('unit_name')+'" data-unit_status="'+$(this).data('unit_status')+'" class="check_box" checked /></td>';
			html += '<td>'+$(this).attr('s_no')+'</td>';
            html += '<td><input type="text" name="unit_name[]" class="form-control field_required form-control-bg-white" value="'+$(this).data("unit_name")+'" /></td>';
            html += '<td><select name="unit_status[]" id="unit_status_'+$(this).attr('id')+'" class="form-control field_required form-control-bg-white"><option value="1">Active</option><option value="0">IN-active</option></select><input type="hidden" name="hidden_id[]" value="'+$(this).attr('id')+'" /></td>';
        ...
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'" s_no="'+$(this).attr('s_no')+'" data-unit_name="'+$(this).data('unit_name')+'" data-unit_status="'+$(this).data('unit_status')+'" class="check_box" /></td>';
			html += '<td>'+$(this).attr('s_no')+'</td>';
            html += '<td>'+$(this).data('unit_name')+'</td>';
			if($(this).data('unit_status') == 1){
				html += '<td><lable class="btn btn-success small-label">Active</label></td>';
			}else{
				html += '<td><lable class="btn btn-danger small-label">In-Active</label></td>';
			}
        ...
        $('#unit_status_'+$(this).attr('id')+'').val($(this).data('unit_status'));
    });
});
...
function update(){
	...
	if(flag == true){
		 if($('.check_box:checked').length > 0){
			 $.ajax({
			   url:"<?php echo base_url();?>index.php/masters/unit/update",
			   method:"POST",
...
function addItemRow(){
	...
	html += '<td></td>';
    html += '<td><input type="text" name="unit_name[]" class="form-control field_required form-control-bg-white" /></td>';
    html += '<td><select name="unit_status[]" class="form-control  form-control-bg-white"><option value="1">Active</option><option value="0">IN-active</option></select><input type="hidden" name="hidden_id[]" value="0" /></td>';
